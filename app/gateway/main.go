...
func main() {
	...
	productConnect, err := grpcx.NewClient("etcd:///product-service-grpc", "round_robin", builder)
	productClient := productv1.NewProductGrpcServiceClient(productConnect)
	defer productConnect.Close()

	orderConnect, err := grpcx.NewClient("etcd:///order-service-grpc", "round_robin", builder)
